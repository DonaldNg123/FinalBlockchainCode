...
# First block --> Genesis block --> Prev_Hash --> 000000000000000
class Blockchain(object):
    ...
    def append_block(self,nonce, Previous_block_hash):
        ...
        return block
    def add_transaction(self, DecentralizedIdentifier,DIDpublickey, Credentials):
        self.current_transaction.append({
            'Decentralized Identifier':DecentralizedIdentifier,
            'DID public key': DIDpublickey,
...
def mine_block():
    ...
    response = {
        ...
        'difficultylevel':blockchain.difficulty_level,
        'time':(end-start),
        'index': block['index'],
...
def new_transactions():
    ...
    required_fields = ['DecentralizedIdentifier', 'DIDpublickey', 'Credentials']
    if not all (k in values for k in required_fields):
        return ('Missing Fields', 400)
    index = blockchain.add_transaction(
        values['DecentralizedIdentifier'],
        values['DIDpublickey'],
        values['Credentials']
        )
